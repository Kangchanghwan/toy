...
data class PaymentEvent(
    ...
    fun completeIfDone() {
        if (allPaymentOrderDone()) {
            isPaymentDone = true
        }
    }

    fun isLedgerUpdateDone(): Boolean {
        return paymentOrders.all { it.isLegerUpdated() }
    }
    ...
    private fun allPaymentOrderDone(): Boolean {
        return paymentOrders.all { it.isWalletUpdated() && it.isLegerUpdated() }
    }
